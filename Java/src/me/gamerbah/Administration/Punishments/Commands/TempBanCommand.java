...
public class TempBanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage(ChatColor.RED + "/tempban <player> <reason>");
            Battlegrounds.playSound(player, EventSound.ACTION_FAIL);
            return true;
        }
        ...
        plugin.createPunishment(targetData.getUuid(), targetData.getName(), Punishment.Type.TEMP_BAN, LocalDateTime.now(), reason.getLength(), player.getUniqueId(), reason);
